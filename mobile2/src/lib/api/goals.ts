import { apiJson, apiBaseUrl } from "@/lib/api/client";
...
export async function topupGoal({ goalId, ...body }: GoalTopupInput): Promise<GoalTopupResult> {
  const res = await apiJson<{ ok: true } & GoalTopupResult>(
...
export function receiptUrl(goalId: string): string {
  return `${apiBaseUrl()}/api/goals/${encodeURIComponent(goalId)}/receipt.pdf`;
}
