...
import { changeCreditLimit } from "@/lib/admin/users";
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  const result = await changeCreditLimit({
  ...
  return NextResponse.json({ ok: true, creditLimitTzs: result.data.creditLimitTzs });
}
